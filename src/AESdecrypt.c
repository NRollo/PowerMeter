...
unsigned char frame_input[sizeof(cipher_text)];

int KamDecrypt(void) {
    int cipherTextlen = sizeof(cipher_text);
    unsigned char cipher[cipherTextlen - headerSize - footerSize - 18 - 12];
    ...
    memcpy(frame_input, cipher_text, sizeof(cipher_text));
    ...
    if (res == 0) {
    memcpy(frame_input + headerSize + 18, output, sizeof(output));
    //mbus_main(output, sizeof(output));
    //mbus_main(frame_input, sizeof(frame_input));
    mbusParseFrame(frame_input, sizeof(frame_input));
    }
    ESP_LOG_BUFFER_HEXDUMP(TAG, frame_input, sizeof(frame_input), ESP_LOG_INFO);
    return res;
}